  ...
  for (let i = 0; i < carNameList.length; i++) {
    ...
    if (carName.length < MIN_CAR_NAME_LENGTH) {
      return new ValidationResult(
        true,
        ERROR_MESSAGES.FALL_SHORT_OF_MIN_CAR_NAME_LENGTH
      );
    }
    if (carName.length > MAX_CAR_NAME_LENGTH) {
      return new ValidationResult(
        true,
        ERROR_MESSAGES.BEYOND_MAX_CAR_NAME_LENGTH
      );
    }
  }
...
export const validateRacingCount = (racingCount) => {
  ...
  if (!isNumber(racingCount)) {
    return new ValidationResult(
      true,
      ERROR_MESSAGES.TYPE_MISMATCH_OF_RACING_COUNT
    );
  }
  if (racingCount <= 0) {
    return new ValidationResult(
      true,
      ERROR_MESSAGES.FALL_SHORT_OF_MIN_RACING_COUNT
    );
  }
